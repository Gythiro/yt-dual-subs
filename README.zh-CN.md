...
> [!IMPORTANT]
> **无统计、无追踪、无账号。** 字幕文本只会发往你选择的服务：当前使用的翻译服务 —— 默认是 YouTube 自身或 Google 翻译，如果你填了自己的 Key，就是你选的那家服务商。设置保存在 `chrome.storage.sync`；你填的 API Key 只存在这台机器的 `chrome.storage.local` 里，不参与同步，除了你选的那家服务商不会发往任何地方。除此之外没有任何数据离开你的浏览器。
...
- **界面也说你的语言** —— 插件自身界面有 20 种语言，自动跟随浏览器，也可以手动切换。

</td>
<td valign="top">

- **高度可定制** —— 每一行的字体（列的是这台电脑上真装了的，也能导入你自己的字体文件）、字号、文字色、背景色 + 不透明度、描边都能单独设置；两行的行距和上下顺序也由你定，弹窗里
- **把视频总结成章节** —— 一句摘要加带时间戳的分章，用你读的语言写，走你自己的 Key 或本机模型；入口在播放器字幕按钮上的小箭头菜单里。
- **三条键盘快捷键** —— 开关字幕、开关朗读、开关译文行；不绑不生效，在 Chrome 的快捷键设置里绑。有实时预览。
