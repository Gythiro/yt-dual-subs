...
> [!IMPORTANT]
> **无统计、无追踪、无账号。** 字幕文本只会发往你选择的服务：当前使用的翻译服务 —— 默认是 YouTube 自身或 Google 翻译，如果你填了自己的 Key，就是你选的那家服务商 —— 以及，只在你打开时，朗读译文的语音服务和你点了「总结」的那家。设置保存在 `chrome.storage.sync`；你填的 API Key 只存在这台机器的 `chrome.storage.local` 里，不参与同步，除了你选的那家服务商不会发往任何地方。其他任何数据都不会离开你的浏览器。
...
<img src="https://gythiro.github.io/yt-dual-subs/img/screenshot-popup.zh.png" width="320" alt="带实时预览的设置弹窗：译文语言、引擎设为自带 Key 并在下方选服务商、以及布局控制">
...
- **50 种译文语言**，显示哪几种由你决定。
- **把译文念出来** —— 每句字幕出现时就朗读它的译文，视频原声自动压低；句子长到读不完时，视频会稍微放慢来配合。用浏览器自带音色**完全不需要 Key**；想要更好的还有五家云服务商，含中文方言音色和你自己克隆的声音。选之前可以先试听。
- **界面也说你的语言** —— 插件自身界面有 44 种语言，自动跟随浏览器，也可以在设置页 →「关于」→「界面语言」里手动切换。
- **六套外观，也可以就用出厂那套** —— 默认、霓虹终端、余烬、摩卡、墨、暮色，在设置页 →「关于」→「界面外观」里选。只影响这个扩展自己，视频上的字幕仍用你给它挑的字体和颜色。

</td>
<td valign="top">

- **高度可定制** —— 每一行的字体（列的是这台电脑上真装了的，也能导入你自己的字体文件）、字号、文字色、背景色 + 不透明度、描边都能单独设置；两行的行距和上下顺序也由你定，弹窗里带实时预览。
- **把视频总结成章节** —— 一句摘要加带时间戳的分章，用你读的语言写，走你自己的 Key 或本机模型；入口在播放器字幕按钮上的小箭头菜单里。
- **三条键盘快捷键** —— 开关字幕、开关朗读、开关译文行；不绑不生效，在 Chrome 的快捷键设置里绑。
- **字幕可以选中复制** —— 默认关着（选中会挡住点画面暂停），在播放器字幕按钮旁小箭头的菜单里打开。
...
- **工具栏图标** → 设置弹窗：译文语言、翻译引擎、上下顺序、位置、行间距，以及每一行的样式，均带实时预览。
- **齿轮图标**（弹窗右上角）→ 独立设置页，六个面：新手入门；翻译服务（自带 Key，或本机模型）；译文语言列表（下拉里显示哪些）；字幕字体（这台电脑上的字体，或导入的文件）；朗读（音色和 Key）；关于（界面语言、快捷键，以及「遇到问题」→「复制诊断信息」）。
...
- 只在 www.youtube.com 本站运行（观看页和 Shorts）。YouTube Music、m.youtube.com，以及嵌在别的网站里的播放器不会出现这层字幕。
...
| `languages.js` | 译文语言表（界面上那个语言数就是从它数出来的） |
...
无统计、无追踪、无账号。字幕文本**仅**发送到你选择的服务：翻译服务，以及——只在你打开时——朗读译文的语音服务和你点了「总结」的那家。设置保存在 `chrome.storage.sync`；你填的 API Key 存在这台机器的 `chrome.storage.local` 里，不参与同步，只作为请求头发给你选定的接口。完整政策见 [PRIVACY.md](PRIVACY.md)。
