...
- **A font file you import** (settings page, "Import font file…") is copied into the extension's own storage in this browser profile (IndexedDB) so it keeps working after the original file moves; it is used only to draw subtitles on this machine, is not synced, is not sent anywhere, and deleting the entry on the settings page deletes the copy.
- **`fontSettings`** — to read the **names of the fonts installed on your computer** (`chrome.fontSettings.getFontList`), so the font picker offers only fonts that really exist there instead of a fixed list of names that may not. The list is read on your machine and used there, to build the picker and to check which of those fonts can draw your subtitle language; it is not sent anywhere. What is stored, in `chrome.storage.local` on that machine only, is your own list of chosen fonts, a small cache of which fonts can draw which subtitle language (so the check is not repeated), and the original languages of the last five videos you watched (so the settings page can start on them); none of it is synced or sent (font lists differ too much between computers to be worth syncing either). The same permission would also allow an extension to *change* the browser's default fonts for every website; this extension does not call any of those methods, and its test suite fails the build if such a call ever appears in the packaged code. Chrome shows no permission warning for this permission, which is why it is spelled out here.
...
- **Optional host access to translation and speech providers** — the extension ships with access to none of them. Each provider's domain (including the endpoints used only for speech — `texttospeech.googleapis.com`, `*.tts.speech.microsoft.com`, `*.tts.speech.azure.cn` and `api.elevenlabs.io` — and the two loopback hosts `http://localhost` / `http://127.0.0.1`, which exist solely so a model server on your own machine can be used and whose traffic does not leave that machine) is listed as an *optional* host permission, and Chrome asks you to grant it at the moment you press "Save and test" for that provider. A provider you never choose is never contacted and never granted anything. A custom endpoint on any other domain is not in this list at all: requests to it are only possible if that server itself opts in via CORS.
